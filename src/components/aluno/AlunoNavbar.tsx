...
const links = [
  ...
  { to: "/aluno/turmas", icon: "fas fa-book-open", label: "Turmas" },
  { to: "/aluno/atividades", icon: "fas fa-tasks", label: "Atividades" },
  ...
  return (
    <nav className="sticky top-0 z-50 w-full px-4 py-2 mb-6 transition-all duration-300 shadow-sm glass-nav">
      <div className="max-w-[1600px] mx-auto px-2 h-20 flex items-center justify-between">
        <NavLink to="/aluno" className="flex items-center gap-3 group">
          <div className="relative flex items-center justify-center w-10 h-10 text-white rounded-xl bg-gradient-to-br from-aurora-primary to-aurora-secondary shadow-neon">
            <i className="text-lg fas fa-user-graduate" />
          </div>
          <div className="flex flex-col">
            <span className="text-lg font-bold tracking-tight text-slate-700 font-display">Hemera</span>
            <span className="text-[9px] font-bold tracking-[0.2em] uppercase text-slate-400">Portal do Aluno</span>
          </div>
        </NavLink>
        ...
        <div className="hidden md:flex items-center bg-white/50 backdrop-blur-md px-2 py-1.5 rounded-full border border-white/50 shadow-inner gap-1">
          {links.map((l) => (
            <NavLink key={l.to} to={l.to} end={l.end}
              className={({ isActive }) => `flex items-center gap-2 px-5 py-2 text-sm font-bold transition-all rounded-full hover:bg-white ${isActive ? "text-aurora-primary bg-white shadow-sm" : "text-slate-600"}`}>
          ...
          <div className="relative">
            <button onClick={() => setOpen((v) => !v)} className="flex items-center gap-3">
              <div className="hidden text-right lg:block">
                ...
                <p className="text-[10px] text-slate-400 font-bold uppercase">Aluno</p>
              </div>
            ...
            {open && (
              <>
                ...
                <div className="absolute right-0 z-50 w-56 p-2 space-y-1 origin-top-right transform border shadow-xl top-12 bg-white/90 backdrop-blur-xl rounded-2xl border-white/50">
                  <NavLink to="/aluno/perfil" className="flex items-center gap-3 px-3 py-2 text-xs font-bold rounded-xl text-slate-600 hover:bg-slate-100"><i className="w-4 fas fa-user" />Meu Perfil</NavLink>
                  <NavLink to="/aluno/configuracoes" className="flex items-center gap-3 px-3 py-2 text-xs font-bold rounded-xl text-slate-600 hover:bg-slate-100"><i className="w-4 fas fa-cog" />Configurações</NavLink>
